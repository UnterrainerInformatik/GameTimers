...
namespace Timer
{
    ...
    public class Fader
    {
        ...
        /// </summary>
        public event EventHandler<ValueChangedArgs<double>> ValueChanged;

        private const double A_THIRD = 1d / 3d;

        private double intervalMin = 0f;
        private double intervalMax = 100f;
        ...
        public double Value
        {
            ...
            set
            {
                ...
                this.value = value;
                percentage = GetPercentageAtValue(value);
                ValueChanged?.Invoke(this, new ValueChangedArgs<double>(oldValue));
            }
        }
        ...
        private double GetValueAtPercentage(double percent)
        {
            if (IsInverted)
            {
                return intervalMin + (1f - percent) * (intervalMax - intervalMin);
            }
            return intervalMin + percent * (intervalMax - intervalMin);
        }
        ...
        private double GetPercentageAtValue(double val)
        {
            if (IsInverted)
            {
                return 1f - ((val - intervalMin) / (intervalMax - intervalMin));
            }
            return (val - intervalMin) / (intervalMax - intervalMin);
        }
    }
}